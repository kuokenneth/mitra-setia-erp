...
import { FiAlertCircle, FiCheck, FiCreditCard, FiFileText, FiPlus, FiPrinter, FiRefreshCw, FiSend, FiX } from "react-icons/fi";
...
export default function Receivables() {
  ...
  const [invoiceForm, setInvoiceForm] = useState({ sourceKey: "", sourceType: "ORDER", orderId: "", materialInvoiceIds: [], singleTripId: "", customerName: "", customerPhone: "", billingAddress: "", dueAt: afterDays(30), contractSubtotal: "", tax: 0, discount: 0, notes: "" });
  ...
  function openInvoice() {
    const source = data.eligibleSources?.[0];
    setInvoiceForm({ sourceKey: source ? `${source.type}:${source.id}` : "", sourceType: source?.type || "ORDER", orderId: source?.type === "ORDER" ? source.id : "", materialInvoiceIds: source?.materialInvoiceIds || [], singleTripId: source?.singleTripId || "", customerName: source?.customerName || "", customerPhone: source?.customerPhone || "", billingAddress: source?.billingAddress || "", dueAt: afterDays(30), contractSubtotal: "", tax: 0, discount: 0, notes: "" });
  ...
  function chooseOrder(sourceKey) {
    ...
    setInvoiceForm(form => ({ ...form, sourceKey, sourceType: source?.type || "ORDER", orderId: source?.type === "ORDER" ? source.id : "", materialInvoiceIds: source?.materialInvoiceIds || [], singleTripId: source?.singleTripId || "", customerName: source?.customerName || "", customerPhone: source?.customerPhone || "", billingAddress: source?.billingAddress || "", contractSubtotal: source?.type === "MATERIAL" ? "0" : "" }));
  }
  ...
  return <div className="ar-page">
    <header className="ar-head"><div><span className="ar-eyebrow">INVOICE & PIUTANG</span><h1>Piutang Pelanggan</h1><p>Pantau penagihan dan pembayaran pelanggan.</p></div><button className="ar-primary" onClick={openInvoice} disabled={!data.eligibleSources?.length}><FiPlus/> Buat Invoice</button></header>
    ...
    {modal === "invoice" && <div className="ar-overlay" onMouseDown={() => setModal("")}><form className="ar-modal ar-invoice-modal" onSubmit={createInvoice} onMouseDown={e => e.stopPropagation()}>
      ...
      <div className="ar-form ar-invoice-form">
        <section className="ar-form-section">
          <div className="ar-section-title"><span>1</span><div><strong>Pilih sumber tagihan</strong><small>Pesanan, trip, dan Faktur Muatan yang telah selesai serta belum ditagih.</small></div></div>
          <label>Sumber tagihan<select required value={invoiceForm.sourceKey} onChange={e => chooseOrder(e.target.value)}><option value="">Pilih sumber tagihan</option>{(data.eligibleSources || []).map(source => <option key={`${source.type}:${source.id}`} value={`${source.type}:${source.id}`}>{source.label}</option>)}</select></label>
        ...
        <section className="ar-form-section">
          ...
          <div className={selectedSource?.type === "MATERIAL" ? "" : "ar-grid2"}><label>Jatuh tempo<input required type="date" value={invoiceForm.dueAt} onChange={e => setInvoiceForm({...invoiceForm, dueAt:e.target.value})}/></label>{selectedSource?.type !== "MATERIAL" && <label>Harga kontrak utama<input required min="0" type="number" inputMode="numeric" value={invoiceForm.contractSubtotal} onChange={e => setInvoiceForm({...invoiceForm, contractSubtotal:e.target.value})} placeholder="Nilai ongkos angkut"/></label>}</div>
      ...
      <div className="ar-modal-actions"><button type="button" className="secondary" onClick={() => setModal("")}>Batal</button><button className="ar-primary" disabled={busy || (selectedSource?.type === "MATERIAL" && !invoiceForm.materialInvoiceIds.length)}>{busy ? "Menyimpan..." : "Simpan Invoice"}</button></div>
    </form></div>}
