...
const initialData = { customers: [], trucks: [], invoices: [], employeeReceivables: [], eligibleOrders: [], eligibleSources: [], stats: { invoiced: 0, received: 0, outstanding: 0, overdue: 0 } };
...
export default function Receivables() {
  ...
  const [employeePayment, setEmployeePayment] = useState(null);
  ...
  function openEmployeePayment(row) {
    setEmployeePayment(row); setPaymentForm({ amount: String(row.balance), method: "BANK_TRANSFER", reference: "", receivedAt: new Date().toISOString().slice(0, 10), notes: "" }); setError(""); setModal("employee-payment");
  }
  ...
  return <div className="ar-page">
    ...
    {employeeView ? <section className="ar-panel ar-employee-panel">
      <div className="ar-panel-heading"><div><span>PIUTANG KARYAWAN</span><h2>Karyawan yang memiliki utang</h2><p>{data.employeeReceivables.length} nama dengan saldo berjalan · tanpa jatuh tempo</p></div><button className="ar-refresh" onClick={load} aria-label="Muat ulang"><FiRefreshCw className={loading ? "ar-spin" : ""}/></button></div>
      <div className="ar-table-wrap"><table className="ar-invoice-table ar-employee-table"><thead><tr><th>Karyawan</th><th>Jumlah Transaksi</th><th>Total Piutang</th><th>Sudah Dibayar</th><th>Sisa</th><th>Tindakan</th></tr></thead><tbody>{data.employeeReceivables.map(row => <tr key={row.employee.id}><td data-label="Karyawan" className="ar-customer-cell"><b>{row.employee.name || "Tanpa nama"}</b><small>{row.employee.email}</small></td><td data-label="Jumlah Transaksi">{row.expenses.length} pengeluaran</td><td data-label="Total Piutang" className="ar-money-cell">{rupiah(row.total)}</td><td data-label="Sudah Dibayar" className="ar-money-cell paid">{rupiah(row.paid)}</td><td data-label="Sisa" className="ar-money-cell ar-balance-cell"><b>{rupiah(row.balance)}</b></td><td data-label="Tindakan" className="ar-action-cell"><div className="ar-actions"><button onClick={() => openEmployeePayment(row)}><FiCreditCard/> Catat Pembayaran</button></div></td></tr>)}</tbody></table></div>
      {!loading && !data.employeeReceivables.length && <div className="ar-empty"><FiCreditCard/><h3>Tidak ada piutang karyawan</h3><p>Nama akan muncul setelah pengeluaran berkategori Piutang Karyawan dibuat.</p></div>}
    </section> :
    ...
    {modal === "employee-payment" && employeePayment && <div className="ar-overlay" onMouseDown={() => setModal("")}><form className="ar-modal ar-payment-modal" onSubmit={saveEmployeePayment} onMouseDown={e => e.stopPropagation()}><div className="ar-modal-head"><div><span className="ar-eyebrow">PIUTANG KARYAWAN</span><h2>Catat pembayaran</h2><p>{employeePayment.employee.name || employeePayment.employee.email}</p></div><button type="button" onClick={() => setModal("")}><FiX/></button></div><div className="ar-form"><div className="ar-balance"><span>Sisa piutang</span><strong>{rupiah(employeePayment.balance)}</strong></div><label>Jumlah diterima<input required min="1" max={employeePayment.balance} type="number" value={paymentForm.amount} onChange={e => setPaymentForm({...paymentForm, amount:e.target.value})}/></label><div className="ar-grid2"><label>Metode<select value={paymentForm.method} onChange={e => setPaymentForm({...paymentForm, method:e.target.value})}><option value="BANK_TRANSFER">Transfer Bank</option><option value="CASH">Tunai</option><option value="OTHER">Lainnya</option></select></label><label>Tanggal diterima<input required type="date" value={paymentForm.receivedAt} onChange={e => setPaymentForm({...paymentForm, receivedAt:e.target.value})}/></label></div><label>Nomor referensi<input value={paymentForm.reference} onChange={e => setPaymentForm({...paymentForm, reference:e.target.value})} placeholder="Nomor transfer atau kuitansi"/></label><label>Catatan<textarea rows="2" value={paymentForm.notes} onChange={e => setPaymentForm({...paymentForm, notes:e.target.value})}/></label></div><div className="ar-modal-actions"><button type="button" className="secondary" onClick={() => setModal("")}>Batal</button><button className="ar-primary" disabled={busy}><FiCheck/> {busy ? "Menyimpan..." : "Simpan Pembayaran"}</button></div></form></div>}
  </div>;
}
