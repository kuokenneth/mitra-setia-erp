...
export default function Purchasing() {
  ...
  return <div className="purchasing">
    ...
    {modal==="receipt"&&receiptPo&&<div className="overlay" onMouseDown={()=>setModal("")}><form className="modal purchase-request-modal transaction-modal" onSubmit={createReceipt} onMouseDown={e=>e.stopPropagation()}><div className="modal-heading"><span className="eyebrow">PENERIMAAN BARANG</span><h2>Terima barang {receiptPo.number}</h2><p>Stok bertambah otomatis setelah penerimaan disimpan.</p></div><div className="po-form-body">
      ...
      <div className="new-item-box"><div className="section-label">INVOICE SUPPLIER</div><div className="grid3"><label>Nomor invoice<input value={receiptForm.supplierInvoiceNumber} onChange={e=>setReceiptForm({...receiptForm,supplierInvoiceNumber:e.target.value})} placeholder="INV-..."/></label><label>Tanggal invoice<input type="date" value={receiptForm.supplierInvoiceDate} onChange={e=>setReceiptForm({...receiptForm,supplierInvoiceDate:e.target.value})}/></label><label>Nilai invoice<input min="0" step="1" type="number" value={receiptForm.supplierInvoiceAmount} onChange={e=>setReceiptForm({...receiptForm,supplierInvoiceAmount:e.target.value})} placeholder="Rp 0"/></label></div><label>Bukti invoice (foto atau PDF, maks. 15 MB)<input type="file" accept="image/*,application/pdf" onChange={e=>setReceiptInvoiceFile(e.target.files?.[0]||null)}/>{receiptInvoiceFile&&<small className="items-loaded">Dipilih: {receiptInvoiceFile.name}</small>}</label></div>
