...
function CatalogItemSearch({ items, value, onChange, disabled, loading }) {
  const selected = items.find(item => item.id === value);
  const [query, setQuery] = useState("");
  const [open, setOpen] = useState(false);
  const normalized = query.trim().toLocaleLowerCase("id-ID");
  const results = useMemo(() => items.filter(item => !normalized || `${item.sku || ""} ${item.name || ""} ${item.unit || ""}`.toLocaleLowerCase("id-ID").includes(normalized)).slice(0, 80), [items, normalized]);
  ...
  return <div className="catalog-item-search">
    <div className={`catalog-item-search-input ${open ? "focused" : ""}`}>
      <FiSearch />
      <input
        disabled={disabled}
        value={query}
        ...
        placeholder={loading ? "Memuat katalog barang..." : selected ? `${selected.sku} — ${selected.name}` : "Cari nama sparepart atau SKU..."}
        aria-label="Cari barang dari katalog inventory"
        autoComplete="off"
      />
      {selected && !query && <button type="button" aria-label="Hapus pilihan barang" onClick={() => { onChange(""); setOpen(true); }}>×</button>}
    </div>
    {selected && !query && <div className="catalog-item-selected"><span><strong>{selected.name}</strong><small>{selected.sku} · {selected.unit || "PCS"}</small></span><b>{Number(selected.qtyTotal || 0).toLocaleString("id-ID")} tersedia</b></div>}
    {open && !disabled && <div className="catalog-item-results">
      {results.map(item => <button type="button" key={item.id} className={item.id === value ? "selected" : ""} onMouseDown={event => event.preventDefault()} onClick={() => choose(item)}><span><strong>{item.name}</strong><small>{item.sku} · {item.unit || "PCS"}</small></span><b>{Number(item.qtyTotal || 0).toLocaleString("id-ID")}</b></button>)}
      {!results.length && <div>Tidak ada sparepart yang cocok.</div>}
    </div>}
  </div>;
}
...
export default function Purchasing() {
  ...
  const [data, setData] = useState({ requests: [], orders: [], items: [], suppliers: [], locations: [], retreadingUnits: [], bills: [], trucks: [], nonSerializedRepairQueue: [] });
  ...
  const [repairReceiptForm, setRepairReceiptForm] = useState({ qty: "", unitPrice: "", locationId: "", notes: "" });
  async function load() {
    setBusy(true);
    try {
  ...
  function openRepairReceipt(repair) {
    setFormError("");
    setRepairReceipt(repair);
    setRepairReceiptForm({ qty: String(repair.remainingQty || ""), unitPrice: "", locationId: repair.toLocationId || data.locations[0]?.id || "", notes: "" });
    setModal("repair-receipt");
  }

  async function receiveNonSerializedRepair(event) {
    event.preventDefault();
    if (!repairReceipt?.id) return;
    setBusy(true); setFormError("");
    try {
      await api(`/purchasing/repairs/non-serialized/${repairReceipt.id}/receive`, { method: "POST", body: JSON.stringify(repairReceiptForm) });
      setModal(""); setRepairReceipt(null); await load();
    } catch (error) { setFormError(error.message || "Gagal menerima hasil perbaikan"); }
    finally { setBusy(false); }
  }
  useEffect(() => {
  ...
  async function createRequest(e) {
    ...
    try {
      if (!newItem && !retreadRequest && !form.itemId) throw new Error("Pilih barang dari katalog inventory");
  ...
  function openPoForm(request) {
    ...
    setPoForm({ supplierId: retreadSupplierId || data.suppliers[0]?.id || "", quantities: Object.fromEntries(request.items.map(item => [item.id, item.tireRetreadId || item.partRepairId ? 1 : request.purpose === "STOCK" ? "" : (item.approvedQty ?? item.originalQty)])), prices: {}, tax: 0, shippingCost: 0, discount: 0, paymentTerms: "Ditagihkan kemudian", deliveryAddress: "", estimatedArrival: "" });
    setModal("po");
  }
    ...
    try {
      ...
      const receiptItems = po => po.items.map(i => ({
          ...
          units: i.item.isSerialized ? [
            ...
            ...String(receiptForm.serials[i.id] || "").split("\n").slice(0, Math.max(0, Math.floor(Number(receiptForm.quantities[i.id] || 0)))).map(x => x.trim()).filter(Boolean).map(serialNumber => ({ serialNumber })),
          ] : undefined,
        })).filter(i => i.qty > 0);
  ...
  function toggleAllBillReceipts() {
    setBillForm(current => {
      const allSelected = selectableBillReceipts.length > 0 && selectableBillReceipts.every(({ receipt }) => current.receiptIds.includes(receipt.id));
      if (allSelected) return { ...current, receiptIds: [], itemIds: [], itemPrices: {} };
      const availableItems = selectableBillReceipts.flatMap(({ receipt }) => receipt.items.filter(item => !item.supplierBillItem));
      return {
        ...current,
        receiptIds: selectableBillReceipts.map(({ receipt }) => receipt.id),
        itemIds: availableItems.map(item => item.id),
        itemPrices: Object.fromEntries(availableItems.map(item => [item.id, item.purchaseOrderItem?.unitPrice > 0 ? String(item.purchaseOrderItem.unitPrice) : ""])),
      };
    });
  }
  function toggleBillItem(item, receipt) {
    setBillForm(current => {
      ...
      const hasSelectedItemInReceipt = receipt.items.some(row => itemIds.includes(row.id));
      const receiptIds = hasSelectedItemInReceipt
        ? [...new Set([...current.receiptIds, receipt.id])]
        : current.receiptIds.filter(id => id !== receipt.id);
      const itemPrices = { ...current.itemPrices };
      ...
      return { ...current, receiptIds, itemIds, itemPrices };
    });
  }
  function toggleBillItemGroup(group) {
    setBillForm(current => {
      const groupIds = group.rows.map(({ item }) => item.id);
      const removing = groupIds.every(id => current.itemIds.includes(id));
      const itemIds = removing ? current.itemIds.filter(id => !groupIds.includes(id)) : [...new Set([...current.itemIds, ...groupIds])];
      const receiptIds = [...new Set(billItemCandidates.filter(({ item }) => itemIds.includes(item.id)).map(({ receipt }) => receipt.id))];
      const itemPrices = { ...current.itemPrices };
      group.rows.forEach(({ item }) => {
        if (removing) delete itemPrices[item.id];
        else if (itemPrices[item.id] === undefined) itemPrices[item.id] = item.purchaseOrderItem?.unitPrice > 0 ? String(item.purchaseOrderItem.unitPrice) : "";
      });
      return { ...current, receiptIds, itemIds, itemPrices };
    });
  }
  function setBillItemGroupPrice(group, value) {
    setBillForm(current => ({
      ...current,
      itemPrices: {
        ...current.itemPrices,
        ...Object.fromEntries(group.rows.filter(({ item }) => current.itemIds.includes(item.id)).map(({ item }) => [item.id, value])),
      },
    }));
  }
  async function createBill(e) {
    ...
    try {
      ...
      if (printPopup) {
        ...
        printPopup.document.write('<!doctype html><html><head><meta charset="utf-8"><title>Menyimpan tagihan…</title></head><body style="font-family:Arial,sans-serif;padding:24px;color:#334c3e">Menyimpan tagihan dan menyiapkan bukti penerimaan…</body></html>');
        printPopup.document.close();
      }
      const proofs = await uploadFiles(billProofFiles);
      const result = await api("/purchasing/bills", { method: "POST", body: JSON.stringify({ ...billForm, proofs }) });
      billSaved = true;
      setModal(""); setTab("payments"); await load();
      if (printPopup) await openPrintDocument(`/purchasing/bills/${result.bill.id}/receipt-print`, printPopup);
    } catch (err) {
      printPopup?.close();
      if (billSaved) window.alert(`Tagihan berhasil disimpan, tetapi dokumen cetak gagal dibuka: ${err.message}`);
  ...
  const allBillReceiptsSelected = selectableBillReceipts.length > 0 && selectableBillReceipts.every(({receipt})=>billForm.receiptIds.includes(receipt.id));
  ...
  const billItemGroups = [...billItemCandidates.reduce((groups, row) => {
    const catalogItem = row.item.purchaseOrderItem?.item;
    const key = row.item.purchaseOrderItem?.itemId || catalogItem?.sku || row.item.id;
    const group = groups.get(key) || { key, item: catalogItem, rows: [] };
    group.rows.push(row);
    groups.set(key, group);
    return groups;
  }, new Map()).values()].sort((a,b)=>String(a.item?.name||"").localeCompare(String(b.item?.name||""),"id"));
  ...
  const receiptGroups = [...data.orders.flatMap(po=>po.receipts.map(gr=>({po,gr}))).reduce((groups, entry) => {
    const proofKey = entry.gr.deliveryNoteProofUrl || entry.gr.deliveryNoteFileName;
    const key = proofKey ? `${entry.po.supplierId}:${proofKey}` : `${entry.po.supplierId}:${entry.gr.deliveryNote || entry.gr.id}`;
    const group = groups.get(key) || { key, entries: [] };
    group.entries.push(entry);
    groups.set(key, group);
    return groups;
  }, new Map()).values()];
  const filteredReceipts = receiptGroups.filter(group=>matchesQuery(group.entries.map(({po,gr})=>[gr.number,gr.deliveryNote,gr.notes,gr.location?.name,po.number,po.supplier?.name,gr.items.map(item=>[item.purchaseOrderItem?.item?.name,item.purchaseOrderItem?.item?.sku])])));
  const filteredRepairReceipts = (data.nonSerializedRepairQueue || []).filter(row=>matchesQuery([row.item?.sku,row.item?.name,row.maintenance?.number,row.maintenance?.title,row.maintenance?.truck?.plateNumber,row.status,row.note]));
  ...
  const pagedRequests = pageSlice(filteredRequests), pagedOrders = pageSlice(filteredOrders), pagedReceipts = pageSlice(filteredReceipts), pagedRepairReceipts = pageSlice(filteredRepairReceipts), pagedBills = pageSlice(filteredBills);
  const currentTotal = ({requests:filteredRequests.length,orders:filteredOrders.length,receipts:filteredReceipts.length,repairReceipts:filteredRepairReceipts.length,payments:filteredBills.length})[tab]||0;
  const searchPlaceholder = ({requests:"Cari nomor permintaan, barang, alasan, atau kendaraan…",orders:"Cari nomor PO, supplier, permintaan, atau barang…",receipts:"Cari nomor penerimaan, PO, supplier, surat jalan, atau barang…",repairReceipts:"Cari barang, servis, atau kendaraan…",payments:"Cari nomor invoice, supplier, penerimaan, atau barang…"})[tab];
  return <div className="purchasing">
    ...
    <nav className="p-tabs"><button className={tab==="requests"?"active":""} onClick={()=>setTab("requests")}>Permintaan Pembelian <i>{data.requests.length}</i></button><button className={tab==="orders"?"active":""} onClick={()=>setTab("orders")}>Pesanan Pembelian <i>{data.orders.length}</i></button><button onClick={()=>setTab("receipts")} className={tab==="receipts"?"active":""}>Penerimaan Barang</button><button onClick={()=>setTab("repairReceipts")} className={tab==="repairReceipts"?"active":""}>Hasil Perbaikan <i>{(data.nonSerializedRepairQueue||[]).filter(row=>row.remainingQty>0).length}</i></button><button onClick={()=>setTab("payments")} className={tab==="payments"?"active":""}>Tagihan & Pembayaran <i>{data.bills.length}</i></button><button className="refresh" onClick={load}><FiRefreshCw className={busy?"spin":""}/></button></nav>
    <section className={`p-panel ${busy && !data.requests.length && !data.orders.length ? "initial-loading" : ""}`}>
      ...
      {tab==="requests" && <div className="table-wrap"><table className="request-table"><thead><tr><th>Nomor</th><th>Kebutuhan</th><th>Bukti foto</th><th>Untuk</th><th>Urgensi</th><th>Status proses</th><th></th></tr></thead><tbody>{pagedRequests.map(r=>{const progress=requestProgress(r,data.orders);const hasPo=data.orders.some(po=>po.request?.id===r.id&&po.status!=="CANCELLED");const requestTruck=data.trucks.find(truck=>truck.id===r.truckId);return <tr key={r.id} className={r.maintenance?"direct-maintenance-row":""}><td><b>{r.number}</b><small>{new Date(r.createdAt).toLocaleDateString("id-ID")}</small>{r.maintenance&&<span className="direct-use-badge">UNTUK SERVIS</span>}{r.approvalNotes?.startsWith("Pembelian langsung")&&<span className="direct-use-badge">PO LANGSUNG</span>}</td><td>{r.items.map(i=><div key={i.id}>{i.item.name} · {r.purpose==="STOCK"&&!i.tireRetreadId&&!i.partRepairId&&Number(i.originalQty)<=0?"Jumlah saat PO":`${i.approvedQty??i.originalQty} ${i.item.unit}`}</div>)}<small>{r.reason}</small></td><td>{r.damageProofUrl?<div className="request-row-proof"><ProtectedFilePreview url={r.damageProofUrl} mimeType={r.damageProofMimeType} fileName={r.damageProofFileName||"Bukti barang rusak"} imageStyle={{width:64,height:48,objectFit:"cover",borderRadius:8}} onError={error=>setFormError(error.message)}/><span><b>Lihat bukti</b><small>Barang rusak</small></span></div>:<span className="request-proof-empty">Tidak ada bukti</span>}</td><td>{r.maintenance?<><b>Servis {r.maintenance?.truck?.plateNumber||"kendaraan"}</b><small>{r.maintenance?.title}</small></>:r.purpose==="TRUCK"?<><b>Truk {requestTruck?.plateNumber||"tertentu"}</b>{requestTruck&&<small>{[requestTruck.brand,requestTruck.model].filter(Boolean).join(" ")}</small>}</>:"Persediaan umum"}</td><td><span className={`urg ${r.urgency}`}>{r.urgency}</span></td><td><span className={`status ${progress.className}`}>{progress.text}</span>{progress.orderNumber&&<small className="request-progress-order">{progress.orderNumber}</small>}{r.status==="APPROVED"&&r.approvedBy?.name&&<small className="approval-person">Disetujui oleh {r.approvedBy.name}</small>}</td><td>{owner&&r.status==="WAITING_APPROVAL"&&<div className="actions request-decision-actions"><button type="button" className="approve" onClick={()=>approve(r,true)}><FiCheck/> Setujui</button><button type="button" className="reject" onClick={()=>approve(r,false)}>Tolak</button></div>}{r.status==="APPROVED"&&!hasPo&&<button type="button" className="create-po-btn" onClick={()=>openPoForm(r)}><FiShoppingCart/> Buat PO</button>}{hasPo&&<button type="button" className="view-po-btn" onClick={()=>setTab("orders")}>Lihat PO <FiChevronRight/></button>}</td></tr>})}</tbody></table>{!busy&&!filteredRequests.length&&<Empty text={normalizedQuery?"Permintaan tidak ditemukan":"Belum ada permintaan pembelian"}/>}</div>}
      ...
      {tab==="receipts" && <div className="receipt-list">{pagedReceipts.map(group=>{const first=group.entries[0];const allItems=group.entries.flatMap(({gr,po})=>gr.items.map(item=>({item,gr,po})));const allBilled=allItems.every(({item})=>item.supplierBillItem);const someBilled=allItems.some(({item})=>item.supplierBillItem);const invoiceCount=new Set(group.entries.flatMap(({gr})=>gr.supplierBillLines.map(line=>line.bill?.id).filter(Boolean))).size;const totalValue=group.entries.reduce((sum,{gr})=>sum+receiptValue(gr),0);const poNumbers=[...new Set(group.entries.map(({po})=>po.number))];return <article className="receipt-card" key={group.key}>
        <header className="receipt-card-head"><div className="receipt-card-identity"><span className="receipt-card-icon"><FiPackage/></span><div><span className="status FULLY_RECEIVED">Barang diterima</span><h3>Surat jalan {first.gr.deliveryNote||"—"}</h3><p>{group.entries.length} penerimaan · {poNumbers.join(", ")} · {first.po.supplier.name}</p></div></div><div className="receipt-card-date"><small>TANGGAL TERIMA</small><time>{new Date(first.gr.receivedAt).toLocaleDateString("id-ID",{day:"numeric",month:"long",year:"numeric"})}</time><span>{[...new Set(group.entries.map(({gr})=>gr.location?.name).filter(Boolean))].join(", ")||"Lokasi tidak dicatat"}</span></div></header>
        <div className="receipt-card-body"><section className="receipt-goods"><header><span>Barang diterima</span><b>{allItems.length} barang · {group.entries.length} GR</b></header><div className="receipt-goods-table"><div className="receipt-goods-heading"><span>Nama barang / Penerimaan</span><span>Jumlah</span></div>{allItems.map(({item,gr,po})=><div className="receipt-goods-row" key={item.id}><span><strong>{item.purchaseOrderItem?.item?.name}</strong><small>{item.purchaseOrderItem?.item?.sku||"Tanpa SKU"} · {gr.number} · {po.number}</small></span><b>{item.qty} {item.purchaseOrderItem?.item?.unit}</b></div>)}</div></section>
          <aside className="receipt-summary"><div className="receipt-summary-row"><span><small>STATUS TAGIHAN</small><strong>{allBilled?`${invoiceCount} invoice · lengkap`:someBilled?`${invoiceCount} invoice · sebagian`:"Belum ditagihkan"}</strong></span><FiFileText/></div><div className="receipt-summary-row"><span><small>NILAI PENERIMAAN</small><strong>{totalValue>0?money(totalValue):"Harga belum tersedia"}</strong></span></div>{first.gr.deliveryNoteProofUrl&&<div className="receipt-proof"><div className="receipt-proof-preview"><ProtectedFilePreview url={first.gr.deliveryNoteProofUrl} mimeType={first.gr.deliveryNoteMimeType} fileName={first.gr.deliveryNoteFileName||"Bukti surat penerimaan"} imageStyle={{width:"100%",height:126,objectFit:"cover"}} onError={e=>setFormError(e.message)}/></div><span><small>BUKTI PENERIMAAN</small><strong title={first.gr.deliveryNoteFileName||"Foto tersimpan"}>{first.gr.deliveryNoteFileName||"Foto tersimpan"}</strong></span></div>}</aside>
        </div>
        <footer className="receipt-card-foot"><span>GR: <b>{group.entries.map(({gr})=>gr.number).join(", ")}</b>{group.entries.some(({gr})=>gr.notes)&&<em>· {group.entries.map(({gr})=>gr.notes).filter(Boolean).join(" · ")}</em>}</span><button type="button" onClick={()=>openPrintDocument(`/purchasing/receipts/group/print?ids=${encodeURIComponent(group.entries.map(({gr})=>gr.id).join(","))}`).catch(err=>setFormError(err.message))}><FiPrinter/> Cetak penerimaan</button></footer>
      ...
      {tab==="repairReceipts" && <div className="table-wrap"><table className="request-table"><thead><tr><th>Dikirim</th><th>Barang</th><th>Servis / Armada</th><th>Jumlah</th><th>Status</th><th>Hasil Penerimaan</th><th></th></tr></thead><tbody>{pagedRepairReceipts.map(repair=><tr key={repair.id}><td>{new Date(repair.createdAt).toLocaleDateString("id-ID",{day:"numeric",month:"long",year:"numeric"})}<small>{repair.createdBy?.name||"—"}</small></td><td><b>{repair.item?.name}</b><small>{repair.item?.sku} → {String(repair.item?.sku||"ITEM").endsWith("_SECOND")?repair.item?.sku:`${repair.item?.sku||"ITEM"}_SECOND`}</small></td><td><b>{repair.maintenance?.truck?.plateNumber||repair.fromTruck?.plateNumber||"—"}</b><small>{repair.maintenance?.number||repair.maintenance?.title||"—"}</small></td><td><b>{repair.receivedQty} / {repair.qty} {repair.item?.unit}</b><small>Sisa {repair.remainingQty} {repair.item?.unit}</small></td><td><span className={`status ${repair.status}`}>{repair.status==="COMPLETED"?"Selesai":repair.status==="PARTIALLY_RECEIVED"?"Diterima sebagian":"Dalam perbaikan"}</span></td><td>{repair.receipts?.length?repair.receipts.map(receipt=><small key={receipt.id}>{receipt.qty} {repair.item?.unit} · {money(receipt.unitPrice)}/unit · {receipt.toLocation?.name}</small>):<small>Belum diterima</small>}</td><td>{repair.remainingQty>0?<button type="button" className="receive" onClick={()=>openRepairReceipt(repair)}><FiPackage/> Terima hasil</button>:<FiCheck/>}</td></tr>)}</tbody></table>{!filteredRepairReceipts.length&&<Empty icon={<FiPackage/>} text={normalizedQuery?"Hasil perbaikan tidak ditemukan":"Belum ada sparepart non-serial dalam perbaikan"}/>}</div>}
    ...
    {modal==="repair-receipt"&&repairReceipt&&<div className="overlay" onMouseDown={()=>setModal("")}><form className="modal purchase-request-modal transaction-modal" onSubmit={receiveNonSerializedRepair} onMouseDown={e=>e.stopPropagation()}><div className="modal-heading"><span className="eyebrow">PENERIMAAN HASIL PERBAIKAN</span><h2>{repairReceipt.item?.name}</h2><p>{repairReceipt.item?.sku} → {String(repairReceipt.item?.sku||"ITEM").endsWith("_SECOND")?repairReceipt.item?.sku:`${repairReceipt.item?.sku||"ITEM"}_SECOND`}</p></div><div className="po-form-body"><div className="po-cancel-warning"><FiPackage/><span><strong>{repairReceipt.maintenance?.truck?.plateNumber||repairReceipt.fromTruck?.plateNumber||"Armada"}</strong><small>Harga stok barang second hanya menggunakan biaya perbaikan yang diisi di bawah.</small></span></div><div className="grid2"><label>Jumlah diterima<input required min="0.01" max={repairReceipt.remainingQty} step="0.01" type="number" value={repairReceiptForm.qty} onChange={e=>setRepairReceiptForm({...repairReceiptForm,qty:e.target.value})}/><small>Maksimal {repairReceipt.remainingQty} {repairReceipt.item?.unit}</small></label><label>Harga perbaikan / unit<input required min="1" step="1" type="number" value={repairReceiptForm.unitPrice} onChange={e=>setRepairReceiptForm({...repairReceiptForm,unitPrice:e.target.value})} placeholder="Rp 0"/></label></div><label>Lokasi Inventory<select required value={repairReceiptForm.locationId} onChange={e=>setRepairReceiptForm({...repairReceiptForm,locationId:e.target.value})}><option value="">Pilih lokasi</option>{data.locations.map(location=><option key={location.id} value={location.id}>{location.name}</option>)}</select></label><label>Catatan<input value={repairReceiptForm.notes} onChange={e=>setRepairReceiptForm({...repairReceiptForm,notes:e.target.value})} placeholder="Kondisi hasil perbaikan"/></label>{formError&&<div className="form-error">{formError}</div>}</div><div className="modal-actions"><button className="secondary-btn" type="button" disabled={busy} onClick={()=>setModal("")}>Batal</button><button className="primary" disabled={busy}>{busy?"Menerima...":"Terima ke Inventory"}</button></div></form></div>}
    {modal==="direct-order"&&<div className="overlay direct-order-overlay" onMouseDown={()=>setModal("")}><form className="modal purchase-request-modal direct-order-modal" onSubmit={createDirectOrder} onMouseDown={e=>e.stopPropagation()}>
      ...
      <footer className="direct-order-footer"><span><FiCheck/> Setelah disimpan, langkah berikutnya hanya menerima barang.</span><div><button className="secondary-btn" type="button" disabled={directBusy} onClick={()=>setModal("")}>Batal</button><button className="primary" disabled={directBusy||itemsLoading}><FiShoppingCart/>{directBusy?"Membuat PO...":"Buat PO & Kirim"}</button></div></footer>
    </form></div>}
    ...
    {modal==="po"&&poRequest&&<div className="overlay" onMouseDown={()=>setModal("")}><form className="modal purchase-request-modal po-modal" onSubmit={createPurchaseOrder} onMouseDown={e=>e.stopPropagation()}><div className="modal-heading"><span className="eyebrow">PESANAN PEMBELIAN</span><h2>Buat PO dari {poRequest.number}</h2><p>Pilih supplier dan pastikan barang serta jumlah yang dipesan.</p></div><div className="po-form-body">
      ...
      <div className="po-price-list"><div className="section-label">BARANG YANG DIPESAN</div>{poRequest.items.map(i=><div className="po-price-row" key={i.id}><div><strong>{i.item.name}</strong><small>{i.item.sku}</small></div>{poRequest.purpose==="STOCK"&&!i.tireRetreadId&&!i.partRepairId?<label>Jumlah PO<input required min="0.01" step="0.01" inputMode="decimal" type="number" value={poForm.quantities[i.id]??""} onChange={e=>setPoForm({...poForm,quantities:{...poForm.quantities,[i.id]:e.target.value}})} placeholder="Masukkan jumlah"/><small>{i.item.unit}</small></label>:<b>{i.approvedQty??i.originalQty} {i.item.unit}</b>}</div>)}</div>
    ...
    {modal==="receipt"&&receiptPo&&<div className="overlay" onMouseDown={()=>setModal("")}><form className="modal purchase-request-modal transaction-modal batch-receipt-modal" onSubmit={createReceipt} onMouseDown={e=>e.stopPropagation()}><div className="modal-heading"><span className="eyebrow">PENERIMAAN BARANG GABUNGAN</span><h2>Terima barang dari {receiptPo.supplier.name}</h2><p>Pilih beberapa PO dari supplier yang sama. Satu surat jalan dan satu bukti dapat digunakan untuk seluruh penerimaan ini.</p>{selectedReceiptOrders.some(po=>(po.request.directUse||po.request.purpose==="MAINTENANCE_STOCK_REQUEST")&&po.request.maintenance)&&<div className="direct-receipt-notice"><strong>LANGSUNG DIPAKAI KE MOBIL</strong><small>{selectedReceiptOrders.filter(po=>(po.request.directUse||po.request.purpose==="MAINTENANCE_STOCK_REQUEST")&&po.request.maintenance).map(po=>`${po.request.maintenance.truck?.plateNumber||"Kendaraan"} · ${po.request.maintenance.title}`).join(" | ")}</small><small>Sistem mencatat IN ke gudang lalu OUT ke mobil servis secara otomatis.</small></div>}</div><div className="po-form-body">
      ...
      <div className="po-price-list receipt-entry-table"><div className="receipt-table-title"><span><small>RINCIAN PENERIMAAN</small><strong>Jumlah diterima sekarang</strong></span><b>{selectedReceiptOrders.reduce((sum,po)=>sum+po.items.length,0)} barang · {selectedReceiptOrders.length} PO</b></div><div className="receipt-table-head"><span>Barang</span><span>Sisa PO</span><span>Qty diterima</span><span>Harga satuan sementara</span></div>{selectedReceiptOrders.flatMap(po=>po.items.map(i=>{
        ...
        const serialQty=Math.max(0,Math.floor(Number(receiptForm.quantities[i.id]||0)));
        const serialRows=Array.from({length:serialQty},(_,index)=>String(receiptForm.serials[i.id]||"").split("\n")[index]||"");
        const typedSerialCount=serialRows.map(x=>x.trim()).filter(Boolean).length;
    ...
    {modal==="bill"&&<div className="overlay" onMouseDown={()=>setModal("")}><form className="modal purchase-request-modal transaction-modal supplier-bill-modal" onSubmit={createBill} onMouseDown={e=>e.stopPropagation()}>
      ...
      <div className="po-form-body"><label>Supplier<select required value={billForm.supplierId} onChange={e=>setBillForm({...billForm,supplierId:e.target.value,receiptIds:[],itemIds:[],itemPrices:{}})}><option value="">Pilih supplier</option>{data.suppliers.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></label>
        ...
        {!!billItemGroups.length&&<div className="bill-item-pricing grouped"><div className="bill-item-row heading"><span>Barang / rincian surat jalan</span><span>Total jumlah</span><span>Harga satuan</span><span>Jumlah harga</span></div>{billItemGroups.map(group=>{const selectedRows=group.rows.filter(({item})=>billForm.itemIds.includes(item.id));const allSelected=selectedRows.length===group.rows.length;const prices=[...new Set(selectedRows.map(({item})=>billForm.itemPrices[item.id]).filter(value=>value!==undefined&&value!==""))];const sharedPrice=prices.length===1?prices[0]:"";const totalQty=selectedRows.reduce((sum,{item})=>sum+Number(item.qty||0),0);const groupTotal=selectedRows.reduce((sum,{item})=>sum+Number(item.qty||0)*Number(billForm.itemPrices[item.id]||0),0);const allowsZero=group.rows.every(({item})=>item.purchaseOrderItem?.partRepairId);return <section className="bill-item-group" key={group.key}><div className={`bill-item-group-head ${selectedRows.length?"selected":"muted"}`}><span className="bill-item-choice"><button type="button" aria-label={`${allSelected?"Batalkan":"Pilih"} semua ${group.item?.name}`} onClick={()=>toggleBillItemGroup(group)}>{allSelected&&<FiCheck/>}</button><span><strong>{group.item?.name}</strong><small>{group.item?.sku} · {group.rows.length} penerimaan</small></span></span><b>{totalQty} {group.item?.unit}</b><input disabled={!selectedRows.length} required={Boolean(selectedRows.length)} min={allowsZero?"0":"1"} step="1" type="number" value={sharedPrice} onChange={event=>setBillItemGroupPrice(group,event.target.value)} placeholder={prices.length>1?"Harga berbeda":"Masukkan harga"}/><strong>{selectedRows.length?money(groupTotal):"—"}</strong></div><div className="bill-item-group-breakdown">{group.rows.map(({item,receipt,po})=>{const selected=billForm.itemIds.includes(item.id);return <button type="button" className={selected?"selected":""} key={item.id} onClick={()=>toggleBillItem(item,receipt)}><i>{selected&&<FiCheck/>}</i><span><strong>Surat jalan {receipt.deliveryNote||"—"}</strong><small>{receipt.number} · {po.number} · {new Date(receipt.receivedAt).toLocaleDateString("id-ID")}</small></span><b>{item.qty} {group.item?.unit}</b></button>})}</div></section>})}</div>}
