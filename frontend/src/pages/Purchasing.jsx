...
export default function Purchasing() {
  ...
  return <div className="purchasing">
    ...
    <section className={`p-panel ${busy && !data.requests.length && !data.orders.length ? "initial-loading" : ""}`}>
      ...
      {tab==="requests" && <div className="table-wrap"><table className="request-table"><thead><tr><th>Nomor</th><th>Kebutuhan</th><th>Bukti foto</th><th>Untuk</th><th>Urgensi</th><th>Status</th><th></th></tr></thead><tbody>{data.requests.map(r=>{const hasPo=data.orders.some(po=>po.request?.id===r.id);return <tr key={r.id} className={r.maintenance?"direct-maintenance-row":""}><td><b>{r.number}</b><small>{new Date(r.createdAt).toLocaleDateString("id-ID")}</small>{r.maintenance&&<span className="direct-use-badge">UNTUK SERVIS</span>}</td><td>{r.items.map(i=><div key={i.id}>{i.item.name} · {i.approvedQty??i.originalQty} {i.item.unit}</div>)}<small>{r.reason}</small></td><td>{r.damageProofUrl?<div className="request-row-proof"><ProtectedFilePreview url={r.damageProofUrl} mimeType={r.damageProofMimeType} fileName={r.damageProofFileName||"Bukti barang rusak"} imageStyle={{width:64,height:48,objectFit:"cover",borderRadius:8}} onError={error=>setFormError(error.message)}/><span><b>Lihat bukti</b><small>Barang rusak</small></span></div>:<span className="request-proof-empty">Tidak ada bukti</span>}</td><td>{r.maintenance?<><b>Servis {r.maintenance?.truck?.plateNumber||"kendaraan"}</b><small>{r.maintenance?.title}</small></>:r.purpose||"Stok umum"}</td><td><span className={`urg ${r.urgency}`}>{r.urgency}</span></td><td><span className={`status ${r.status}`}>{hasPo?"PO dibuat":label(r.status)}</span>{r.status==="APPROVED"&&r.approvedBy?.name&&<small className="approval-person">Disetujui oleh {r.approvedBy.name}</small>}</td><td>{owner&&r.status==="WAITING_APPROVAL"&&<div className="actions request-decision-actions"><button type="button" className="approve" onClick={()=>approve(r,true)}><FiCheck/> Setujui</button><button type="button" className="reject" onClick={()=>approve(r,false)}>Tolak</button></div>}{r.status==="APPROVED"&&!hasPo&&<button type="button" className="create-po-btn" onClick={()=>openPoForm(r)}><FiShoppingCart/> Buat PO</button>}{hasPo&&<button type="button" className="view-po-btn" onClick={()=>setTab("orders")}>Lihat PO <FiChevronRight/></button>}</td></tr>})}</tbody></table>{!busy&&!data.requests.length&&<Empty text="Belum ada permintaan pembelian"/>}</div>}
      {tab==="orders" && <div className="cards">{data.orders.map(po=>{const total=poTotal(po);return <article className="po-card" key={po.id}><div><span className={`status ${po.status}`}>{label(po.status)}</span><h3>{po.number}</h3><p>{po.supplier.name} · dari {po.request.number}</p></div><div className="po-items">{po.items.map(i=><span key={i.id}>{i.item.name}<b>{i.receivedQty}/{i.qty} {i.item.unit}</b></span>)}</div><div className="po-progress"><span style={{width:`${Math.min(100,po.items.reduce((s,i)=>s+i.receivedQty,0)/Math.max(1,po.items.reduce((s,i)=>s+i.qty,0))*100)}%`}}/></div><footer><div><strong>{total>0?money(total):"Harga menyusul"}</strong><small>{total>0?"Sesuai tagihan supplier":po.paymentTerms||"Ditagihkan kemudian"}</small></div></footer><div className="po-actions"><button type="button" onClick={()=>openPrintDocument(`/purchasing/orders/${po.id}/print`).catch(err=>setFormError(err.message))}><FiPrinter/> Print PO</button>{owner&&po.status==="DRAFT"&&<button type="button" onClick={()=>changePoStatus(po,"APPROVED")}><FiCheck/> Setujui PO</button>}{owner&&po.status==="APPROVED"&&<button type="button" onClick={()=>changePoStatus(po,"SENT_TO_SUPPLIER")}>Tandai dikirim</button>}{["SENT_TO_SUPPLIER","PARTIALLY_RECEIVED"].includes(po.status)&&<button type="button" className="receive" onClick={()=>openReceiptForm(po)}><FiPackage/> Terima barang</button>}</div></article>})}{!data.orders.length&&<Empty text="PO akan tampil setelah permintaan disetujui"/>}</div>}
