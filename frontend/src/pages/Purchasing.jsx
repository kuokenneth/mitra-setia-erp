...
export default function Purchasing() {
  ...
  const [receiptForm, setReceiptForm] = useState({ locationId: "", locationName: "Gudang Utama", quantities: {}, serials: {}, retreadUnitIds: {}, repairUnitIds: {}, deliveryNote: "", notes: "" });
  ...
  const [billForm, setBillForm] = useState({ supplierId: "", invoiceNumber: "", invoiceDate: "", dueDate: "", notes: "", receiptIds: [], itemPrices: {} });
  ...
  function openBillForm() {
    ...
    setBillForm({ supplierId: first?.id || "", invoiceNumber: "", invoiceDate: new Date().toISOString().slice(0, 10), dueDate: "", notes: "", receiptIds: [], itemPrices: {} });
  ...
  function toggleBillReceipt(receipt) {
    setBillForm(current => {
      ...
      if (removing) receipt.items.forEach(item => delete itemPrices[item.id]);
      else receipt.items.forEach(item => { itemPrices[item.id] = item.purchaseOrderItem?.unitPrice > 0 ? String(item.purchaseOrderItem.unitPrice) : ""; });
      return { ...current, receiptIds: selected, itemPrices };
    });
  }
  async function createBill(e) {
    e.preventDefault(); setBusy(true); setFormError("");
    try {
      if (!billProofFile) throw new Error("Lampirkan invoice supplier untuk proses pencocokan");
      const uploaded = (await uploadFiles([billProofFile]))[0];
      await api("/purchasing/bills", { method: "POST", body: JSON.stringify({ ...billForm, proofUrl: uploaded?.url, proofFileName: uploaded?.fileName, proofMimeType: uploaded?.mimeType, proofSize: uploaded?.size }) });
  ...
  const billItems = billReceipts.flatMap(({receipt,po})=>receipt.items.map(item=>({item,receipt,po})));
  ...
  return <div className="purchasing">
    ...
    <section className={`p-panel ${busy && !data.requests.length && !data.orders.length ? "initial-loading" : ""}`}>
      ...
      {tab==="receipts" && <div className="receipt-list">{data.orders.flatMap(po=>po.receipts.map(gr=><article className="receipt-card" key={gr.id}>
        ...
        <div className="receipt-card-main"><div className="receipt-card-title"><div><span className="status FULLY_RECEIVED">Barang diterima</span><h3>{gr.number}</h3></div><time>{new Date(gr.receivedAt).toLocaleDateString("id-ID",{day:"numeric",month:"long",year:"numeric"})}</time></div><p className="receipt-source">{po.number}<span>•</span>{po.supplier.name}<span>•</span>{gr.location?.name}</p>
          <div className="receipt-info"><div><FiFileText/><span><small>Status tagihan</small><strong>{gr.supplierBillLine?`Masuk ${gr.supplierBillLine.bill.invoiceNumber}`:"Menunggu tagihan supplier"}</strong></span></div><div><span><small>Nilai barang diterima</small><strong>{receiptValue(gr)>0?money(receiptValue(gr)):"Menunggu harga supplier"}</strong></span></div></div>
    ...
    {modal==="request"&&<div className="overlay" onMouseDown={()=>setModal("")}><form className="modal purchase-request-modal request-redesign-modal" onSubmit={createRequest} onMouseDown={e=>e.stopPropagation()}>
      ...
      <div className="request-modal-body">
        ...
        <label className={`request-damage-proof ${requestDamagePhoto?"has-file":""}`}><input required type="file" accept="image/*" capture="environment" onChange={e=>setRequestDamagePhoto(e.target.files?.[0]||null)}/><span className="request-proof-icon"><FiUploadCloud/></span><span className="request-proof-copy"><strong>{requestDamagePhoto?"Foto siap dikirim":"Ketuk untuk tambah foto barang rusak"}</strong><small>{requestDamagePhoto?`${requestDamagePhoto.name} · Ketuk kembali untuk mengganti`:"Gunakan kamera atau pilih foto dari galeri"}</small></span></label>
    ...
    {modal==="bill"&&<div className="overlay" onMouseDown={()=>setModal("")}><form className="modal purchase-request-modal transaction-modal supplier-bill-modal" onSubmit={createBill} onMouseDown={e=>e.stopPropagation()}>
      ...
      <div className="po-form-body"><label>Supplier<select required value={billForm.supplierId} onChange={e=>setBillForm({...billForm,supplierId:e.target.value,receiptIds:[],itemPrices:{}})}><option value="">Pilih supplier</option>{data.suppliers.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></label>
        <div className="bill-receipt-picker">{data.orders.filter(po=>po.supplierId===billForm.supplierId).flatMap(po=>po.receipts.map(receipt=>({receipt,po}))).filter(({receipt})=>!receipt.supplierBillLine).map(({receipt,po})=><label key={receipt.id} className={billForm.receiptIds.includes(receipt.id)?"selected":""}><input type="checkbox" checked={billForm.receiptIds.includes(receipt.id)} onChange={()=>toggleBillReceipt(receipt)}/><span><strong>{receipt.number}</strong><small>{po.number} · Surat jalan {receipt.deliveryNote||"-"} · {new Date(receipt.receivedAt).toLocaleDateString("id-ID")}</small></span><b>{receipt.items.length} barang</b></label>)}{billForm.supplierId&&!data.orders.filter(po=>po.supplierId===billForm.supplierId).some(po=>po.receipts.some(receipt=>!receipt.supplierBillLine))&&<small>Semua penerimaan supplier ini sudah ditagihkan.</small>}</div>
        ...
        {!!billItems.length&&<div className="bill-item-pricing"><div className="bill-item-row heading"><span>Barang / Penerimaan</span><span>Jumlah</span><span>Harga satuan</span><span>Jumlah harga</span></div>{billItems.map(({item,receipt})=><div className="bill-item-row" key={item.id}><span><strong>{item.purchaseOrderItem?.item?.name}</strong><small>{receipt.number} · {item.purchaseOrderItem?.item?.sku}{item.purchaseOrderItem?.partRepairId ? " · Perbaikan (boleh Rp0)" : ""}</small></span><b>{item.qty} {item.purchaseOrderItem?.item?.unit}</b><input required min={item.purchaseOrderItem?.partRepairId?"0":"1"} step="1" type="number" value={billForm.itemPrices[item.id]??""} onChange={e=>setBillForm(current=>({...current,itemPrices:{...current.itemPrices,[item.id]:e.target.value}}))} placeholder="Rp 0"/><strong>{money(Number(item.qty||0)*Number(billForm.itemPrices[item.id]||0))}</strong></div>)}</div>}
        ...
        <label className="receipt-upload required-proof"><input required type="file" accept="image/jpeg,image/png,image/webp,application/pdf" onChange={e=>setBillProofFile(e.target.files?.[0]||null)}/><span className="receipt-upload-icon"><FiUploadCloud/></span><span><strong>{billProofFile?billProofFile.name:"Lampirkan invoice supplier"}</strong><small>Wajib · JPG, PNG, WEBP atau PDF · maksimal 15 MB</small></span></label><label>Catatan<textarea rows="2" value={billForm.notes} onChange={e=>setBillForm({...billForm,notes:e.target.value})}/></label>{formError&&<div className="form-error">{formError}</div>}
      </div><div className="modal-actions"><button className="secondary-btn" type="button" onClick={()=>setModal("")}>Batal</button><button className="primary" disabled={busy||!billPricesValid||!billProofFile}>{busy?"Mencocokkan...":"Cocok & Simpan Tagihan"}</button></div></form></div>}
