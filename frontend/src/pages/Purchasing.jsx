...
export default function Purchasing() {
  ...
  const [data, setData] = useState({ requests: [], orders: [], items: [], suppliers: [], locations: [], retreadingUnits: [] });
  ...
  const [receiptForm, setReceiptForm] = useState({ locationId: "", locationName: "Gudang Utama", quantities: {}, serials: {}, retreadUnitIds: {}, deliveryNote: "", notes: "", supplierInvoiceNumber: "", supplierInvoiceDate: "", supplierInvoiceAmount: "" });
  const [receiptInvoiceFile, setReceiptInvoiceFile] = useState(null);
  ...
  function openReceiptForm(po) {
    ...
    setReceiptForm({ locationId: data.locations[0]?.id || "", locationName: "Gudang Utama", quantities: Object.fromEntries(po.items.map(i => [i.id, i.qty - i.receivedQty])), serials: Object.fromEntries(po.items.map(i => [i.id, ""])), retreadUnitIds: Object.fromEntries(po.items.map(i => [i.id, i.tireRetread?.stockUnitId ? [i.tireRetread.stockUnitId] : []])), deliveryNote: "", notes: "", supplierInvoiceNumber: "", supplierInvoiceDate: "", supplierInvoiceAmount: String(poTotal(po)) });
    setReceiptInvoiceFile(null); setModal("receipt");
  }
  async function createReceipt(e) {
    ...
    try {
      ...
      const uploaded = receiptInvoiceFile ? (await uploadFiles([receiptInvoiceFile]))[0] : null;
      await api("/purchasing/receipts", { method: "POST", body: JSON.stringify({ purchaseOrderId: receiptPo.id, locationId, deliveryNote: receiptForm.deliveryNote, notes: receiptForm.notes, supplierInvoiceNumber: receiptForm.supplierInvoiceNumber, supplierInvoiceDate: receiptForm.supplierInvoiceDate || null, supplierInvoiceAmount: receiptForm.supplierInvoiceAmount, supplierInvoiceProofUrl: uploaded?.url, supplierInvoiceFileName: uploaded?.fileName, supplierInvoiceMimeType: uploaded?.mimeType, supplierInvoiceSize: uploaded?.size, items: receiptPo.items.map(i => ({
  ...
  return <div className="purchasing">
    ...
    <section className="p-panel">
      ...
      {tab==="receipts" && <div className="receipt-list">{data.orders.flatMap(po=>po.receipts.map(gr=>{const total=poTotal(po);const committed=po.payments.filter(p=>p.status!=="UNPAID").reduce((s,p)=>s+p.amount,0);return <article className="receipt-card" key={gr.id}>
        ...
        <div className="receipt-card-main"><div className="receipt-card-title"><div><span className="status FULLY_RECEIVED">Barang diterima</span><h3>{gr.number}</h3></div><time>{new Date(gr.receivedAt).toLocaleDateString("id-ID",{day:"numeric",month:"long",year:"numeric"})}</time></div><p className="receipt-source">{po.number}<span>•</span>{po.supplier.name}<span>•</span>{gr.location?.name}</p>
          <div className="receipt-info"><div><FiFileText/><span><small>Invoice supplier</small><strong>{gr.supplierInvoiceNumber||"Belum dicatat"}</strong></span></div><div><span><small>Nilai invoice</small><strong>{gr.supplierInvoiceAmount!=null?money(gr.supplierInvoiceAmount):"—"}</strong></span></div></div>
          {gr.supplierInvoiceProofUrl&&<div className="receipt-proof"><ProtectedFilePreview url={gr.supplierInvoiceProofUrl} mimeType={gr.supplierInvoiceMimeType} fileName={gr.supplierInvoiceFileName||"Bukti invoice"} imageStyle={{width:112,height:72,objectFit:"cover",borderRadius:8}} onError={e=>setFormError(e.message)}/></div>}
          ...
          <div className="receipt-actions"><button type="button" onClick={()=>openPrintDocument(`/purchasing/receipts/${gr.id}/print`).catch(err=>setFormError(err.message))}><FiPrinter/> Print penerimaan</button>{["PARTIALLY_RECEIVED","FULLY_RECEIVED"].includes(po.status)&&total>committed&&<button type="button" className="primary-action" onClick={()=>openPaymentForm(po)}><FiShoppingCart/> Ajukan bayar</button>}</div>
        </div>
    ...
    {modal==="receipt"&&receiptPo&&<div className="overlay" onMouseDown={()=>setModal("")}><form className="modal purchase-request-modal transaction-modal" onSubmit={createReceipt} onMouseDown={e=>e.stopPropagation()}><div className="modal-heading"><span className="eyebrow">PENERIMAAN BARANG</span><h2>Terima barang {receiptPo.number}</h2><p>Stok bertambah otomatis setelah penerimaan disimpan.</p></div><div className="po-form-body">
      ...
      <div className="new-item-box invoice-entry-card"><div className="section-label">INVOICE SUPPLIER</div><div className="grid3"><label>Nomor invoice<input value={receiptForm.supplierInvoiceNumber} onChange={e=>setReceiptForm({...receiptForm,supplierInvoiceNumber:e.target.value})} placeholder="INV-..."/></label><label>Tanggal invoice<input type="date" value={receiptForm.supplierInvoiceDate} onChange={e=>setReceiptForm({...receiptForm,supplierInvoiceDate:e.target.value})}/></label><label>Nilai invoice<input min="0" step="1" type="number" value={receiptForm.supplierInvoiceAmount} onChange={e=>setReceiptForm({...receiptForm,supplierInvoiceAmount:e.target.value})} placeholder="Rp 0"/></label></div><label className="receipt-upload"><input type="file" accept="image/jpeg,image/png,image/webp,application/pdf" onChange={e=>setReceiptInvoiceFile(e.target.files?.[0]||null)}/><span className="receipt-upload-icon"><FiUploadCloud/></span><span><strong>{receiptInvoiceFile?receiptInvoiceFile.name:"Pilih bukti invoice"}</strong><small>{receiptInvoiceFile?"Klik untuk mengganti file":"JPG, PNG, WEBP atau PDF · maksimal 15 MB"}</small></span></label></div>
