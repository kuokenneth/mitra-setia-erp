    ...
    {selected && <div className="fp-overlay" onMouseDown={event => event.target === event.currentTarget && setSelected(null)}><section className="fp-detail-modal">
      ...
      <div className="fp-detail-body">
        ...
        <div className="fp-detail-columns">
          ...
          <div className="fp-detail-section"><div className="fp-detail-title"><div><FiActivity/><h3>Sparepart Terpasang</h3></div><strong>{money(selected.spareParts)}</strong></div><div className="fp-cost-list">{(selected.sparePartDetails || []).map(part => <div key={part.id}><span><b>{part.isReturn ? `Pengembalian ${part.name}` : part.name}</b><small>{part.sku || "Tanpa SKU"}{part.quantity != null ? ` · ${part.quantity} ${part.unit || "unit"}` : ""} · {date(part.installedAt)}{part.maintenanceTitle ? ` · ${part.maintenanceTitle}` : ""}</small></span><strong>{money(part.cost)}</strong></div>)}{!selected.sparePartDetails?.length && <p>Belum ada biaya sparepart bulan ini.</p>}</div></div>
          <div className="fp-detail-section"><div className="fp-detail-title"><div><FiFileText/><h3>Pengeluaran Langsung Armada</h3></div><strong>{money(selected.vehicleExpenses)}</strong></div><div className="fp-cost-list">{(selected.vehicleExpenseDetails || []).map(item => <div key={item.id}><span><b>{item.reason}</b><small>{date(item.expenseDate)} · {item.status}</small></span><strong>{money(item.amount)}</strong></div>)}{!selected.vehicleExpenseDetails?.length && <p>Belum ada pengeluaran langsung armada bulan ini.</p>}</div></div>
